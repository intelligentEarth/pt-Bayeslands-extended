...
class ptReplica(multiprocessing.Process):
    ...
    def run_badlands_nathan(self, input_vector):
        ...
        #elev_updated=filters.gaussian_filter(elev,input_vector[0]) #+1*input_vector[1]
        elev_updated=filters.gaussian_filter(elev,input_vector[0]) + self.simtime*input_vector[1]/1000
        print("uplifting by ",self.simtime*input_vector[1]/1000," m")
        #plt.imshow(elev_updated)
        #plt.savefig(problem_folder+str(self.run_nb)+'/fig_'+str(round(input_vector[0],1))+'_off_'+str(round(input_vector[1],1))+'.png')
        
        ##METHOD 2
        #If we use just 'index numbers' to specify what coordinates to fix, do this
        #List of fixed coordinates
        #fixed_coords = np.array([[41,10],[41,9],[10,51],[59,13],[40,5],[6,20],[14,20],[4,40],[30,30],[46,30]])
        #elev_updated = elev

        ##Make the vectors of the index values and make an array of them
        #xi = np.linspace(0, model.recGrid.nx-1, model.recGrid.rnx)
        #yi = np.linspace(0, model.recGrid.ny-1, model.recGrid.rny)
        #xi, yi = np.meshgrid(xi, yi)
        #xyi = np.dstack([xi.flatten(), yi.flatten()])[0]

        ##Define a fucntion to find the closest "fixed point" in the model
        #def closest_node(node,nodes):
            #from scipy.spatial import distance
            #closest_index=distance.cdist([node],nodes,'euclidean').argmin()
            #return closest_index

        ##Find the closest fixed point for our grid 
        #for i in xyi:
            #closest_index = closest_node(i,fixed_coords)
            
            ##and apply the updated topography algorithm for each point
            #elev_updated[int(i[0]),int(i[1])]=(model.recGrid.rectZ[closest_index]+elev[int(i[0]),int(i[1])])/2 + random.randint(-1,1)*input_vector[0]
        
        ##Finally, smooth the result
        #elev_updated=filters.gaussian_filter(elev_updated,2)
        ...
        # Adjust precipitation values based on given parameter - can be time dependant
        #model.force.rainVal[:] = input_vector[0:3] 

        # Adjust m and n values
        #model.input.SPLm = input_vector[2] 
        #model.input.SPLn = input_vector[3] 

        if sys.argv[1]==3 or sys.argv[1]==4:  # will work for more parameters
        ...
        if sys.argv[1]==6:
            k=round(input_vector[4],1) #round(input_vector[4]*2)/2
            ...
            #model.force.Map_disp=model.input.tectFile
...
class ParallelTempering:
    ...
    def __init__(self, vec_parameters, rain_region, rain_time, num_chains, maxtemp,NumSample,swap_interval, fname, realvalues_vec, num_param,  real_elev, erodep_pts, erodep_coords, simtime, siminterval, resolu_factor, run_nb, inputxml):
    ...
    def initialize_chains (self,     minlimits_vec, maxlimits_vec, stepratio_vec,  check_likelihood_sed,   burn_in):
        ...
        for i in xrange(0, self.num_chains):
            self.chains.append(ptReplica( self.num_param, self.vec_parameters, self.rain_region, self.rain_time, minlimits_vec, maxlimits_vec, stepratio_vec,  check_likelihood_sed ,self.swap_interval, self.sim_interval,   self.simtime, self.NumSamples, self.real_elev,   self.real_erodep_pts, self.erodep_coords, self.folder, self.xmlinput,  self.run_nb,self.temperatures[i], self.parameter_queue[i],self.event[i], self.wait_chain[i],burn_in))
...
def main():
    ...
    if problem == 1: #this is CM-extended
        ...
        erodep_coords = np.array([[42,10],[39,8],[75,51],[59,13],[40,5],[6,20],[14,66],[4,40],[72,73],[46,64]])  # need to be updated as in Surrogates paper
        if (true_parameter_vec.shape[0] != vec_parameters.size ) :
            print( 'vec_params != true_values.txt ',true_parameter_vec.shape,vec_parameters.size)
            print( 'make sure that this is updated in case when you intro more parameters. should have as many rows as parameters ') 
    ...
    #-------------------------------------------------------------------------------------
    pt = ParallelTempering(  vec_parameters, rain_regiongrid, rain_timescale, num_chains, maxtemp, samples,swap_interval,fname, true_parameter_vec, num_param  ,  groundtruth_elev,  groundtruth_erodep_pts , erodep_coords, simtime, sim_interval, resolu_factor, run_nb_str, xmlinput)
