...
def topoGenerator(directory, inputname, rain, erodibility, m, n, simtime, erdp_coords, final_noise):
    ...
    print(str(simtime), inputname)
    model = badlandsModel()
    model.load_xml(str(simtime), inputname, verbose = True, muted = True)
    ...
    #tectonicValues=pd.read_csv(str(model.input.tectFile[0]),sep=r'\s+',header=None,dtype=np.float).values
    #newtect = pd.DataFrame(tectonicValues/5.)
    #newFile = "Examples/mountain/data/uplift_small.csv"
...
def main():
    ...
    erdp_coords = np.array([[42,10],[39,8],[10,51],[59,13],[40,5],[6,20],[14,20],[4,40],[30,30],[46,30]])
    ...
    topoGenerator(directory,'%s/png.xml' %(directory), 1 , 7.e-6, 0.5, 1, 60000, erdp_coords,final_noise)
        
    print 'TopoGen completed in (s):',time.clock()-tstart
